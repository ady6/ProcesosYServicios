...
public class TestRMI extends UnicastRemoteObject implements ITestRMI {
	...
	public ArrayList<Pregunta> get5Azar() throws RemoteException {
		...
		// Carga el objeto preguntas5 con 5 preguntas al azar que no se repitan.
		ArrayList<Pregunta> preguntas = getPreguntas(); //array de todas las preguntas
		
		//copia de array de todas las preguntas para no eliminar las preguntas del array original
		ArrayList<Pregunta> preguntas1 = (ArrayList<Pregunta>) preguntas.clone();
		
		for(int i = 0; i<5; i++) { //bucle para 5 preguntas
			byte numeroazar = (byte) (Math.random()*(10-i));
			Pregunta pregunta = preguntas1.get(numeroazar);
			preguntas5.add(pregunta);
			preguntas1.remove(numeroazar); //elimino indice (numeroazar) del array clonado para evitar repeticiones
			}
			return preguntas5;
		}
	}
